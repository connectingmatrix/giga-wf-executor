...
describe('shared workflow executor', () => {
    it('returns validation error when start/end nodes are missing', async () => {
        ...
        expect(result.logs.some((item) => item.event === 'workflow.validation_failed')).toBe(true);
    });
    ...
    it('executes true branch and sends summary to end node', async () => {
        ...
        const handlers = createAdapters({
            ...
            'if-else': async (_node, input) => {
                ...
                return { output: { passed: started, __activeOutputs: [started ? 'true' : 'false'] }, status: WorkflowNodeStatusEnum.Passed };
            },
        ...
        const executor = createWorkflowExecutor({ mode: WorkflowExecutorModeEnum.Local, adapters: handlers });
        const logger = createJsonlLogger();
        const result = await executor.executeWorkflow(workflow, {
            settings: { graphqlUrl: 'http://localhost/graphql', authMode: 'none' },
            logger
        ...
        expect((endAfter?.output as { summary?: unknown })?.summary).toBeTruthy();
    });
});
